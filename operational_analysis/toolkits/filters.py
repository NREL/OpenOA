...
def std_range_flag(data_col, threshold = 2.):
    ...
    flag = ((data_col <= data_mean - threshold*data_std) | (data_col >= data_mean + threshold*data_std)) # Apply the range flag
...
def bin_filter(bin_col, value_col, bin_width, threshold = 2, center_type = 'mean', bin_min = None, bin_max = None, threshold_type = 'std', direction = 'all'):
    ...
    for n in np.arange(nbins):
         ...
         if center_type == 'mean': cent = y_bin.mean()
         elif center_type == 'median': cent = y_bin.median()
         else: print 'incorrect center type specified'
         
         # Define threshold of data flag
         if threshold_type == 'std': ran = y_bin.std()*threshold
         elif threshold_type == 'scalar': ran = threshold
...
def cluster_mahalanobis_2d(data_col1, data_col2, n_clusters = 13, dist_thresh = 3.):
    ...
    for ic in range(n_clusters):
        ...
        mahalanobis_dist = cluster.apply(lambda r: sp.spatial.distance.mahalanobis(r.values, centroid, invcovmx), axis=1)
