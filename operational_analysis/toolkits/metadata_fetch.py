...
def fetch_eia(api_key, plant_id, file_path):
	...
	def meta_dic_fn(file, sheet, var_list): 
    
		all_plant = pd.read_excel(file_path+file, sheet_name=sheet, skiprows=1)
	
		eia_plant = all_plant.loc[all_plant['Plant Code']==np.int(plant_id)] # specific wind farm 
		eia_info = eia_plant.loc[:, lambda eia_plant:var_list] # select columns
	...
	hubheight_meter = unit_conversion.convert_feet_to_meter(wind_dic['Turbine Hub Height (Feet)']) 
