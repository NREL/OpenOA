...
def correlation_matrix_by_id_column(align_col, id_col, value_col):
    """Create a correlation matrix between different assets in a data frame

    Args:
        align_col(:obj:`array-like`): array of different assets are to be aligned
        id_col(:obj:`array-like`): array distinguishing the different assets
        value_col(:obj:`array-like`): array containing the data values to be used when assessing correlation

    Returns:
        :obj:`pandas.DataFrame`: Correlation matrix with <id_col> as index and column names
    """
    assets = np.unique(id_col)  # List different assets in <id_col>
    ...
    df = pd.DataFrame(np.column_stack((id_col, align_col, value_col)), columns=["id_col", "align_col", "value_col"])
    ...
    for t in assets:  # Loop through assets
        ...
        for s in assets:  # Loop through remaining assets
            ...
            if (merged_df.empty) | (merged_df.shape[0] < 2):
                ...
                corr_df.loc[t, s] = np.corrcoef(merged_df['value_col_x'], merged_df['value_col_y'])[0, 1]
...
def impute_data(target_value_col, target_align_col, ref_value_col, ref_align_col, index_col=None,
    ...
    Args:
        target_value_col(:obj:`array-like`): the target array to be imputed
        target_align_col(:obj:`array-like`): the array on which different assets are to be merged
        ref_value_col(:obj:`array-like`): the reference array to be used in imputation
        ref_align_col(:obj:`array-like`): the array on which different assets are to be merged
        index_col(:obj:`array-like`): the array of indices to be used.

    Returns:
        :obj:`pandas.Series`: Copy of target_data_col series with NaN occurrences imputed where possible.
    """
    # Create the base data frames
    target_df = pd.DataFrame(np.column_stack((target_value_col, target_align_col)), columns=["target_value", "align_col"])
    target_df.target_value = target_df.target_value.astype(float)
    ref_df = pd.DataFrame(np.column_stack((ref_value_col, ref_align_col)), columns=["ref_value", "align_col"])
    ref_df.ref_value = ref_df.ref_value.astype(float)

    # Merge the input and reference data frames, keeping the input data indices
    merge_df = pd.merge(target_df, ref_df, on="align_col", how='left')
    
    merge_df = merge_df.set_index(target_df.index if index_col is None else index_col)
    ...
    if method == 'linear':
        reg = np.polyfit(reg_df.ref_value, reg_df.target_value, 1)  # Linear regression relationship
        slope = reg[0]  # Slope
    ...
    impute_df = merge_df.loc[(merge_df.target_value.isnull()) & np.isfinite(merge_df.ref_value)]
    imputed_data = slope * impute_df.ref_value + intercept
...
def impute_all_assets_by_correlation(
    input_col, ref_col, align_col, id_col, r2_threshold=0.7, index_col=None, method='linear'
):
    ...
    Args:
        input_col(:obj:`array-like`): the array to be imputed
        ref_col(:obj:`array-like`): the array to be used in imputation
        align_col(:obj:`array-like`): the array on which different assets are to be merged
        id_col(:obj:`array-like`): the array distinguishing different assets
        r2_threshold(:obj:`float`): the correlation threshold for a neighboring assets to be considered valid
                                   for use in imputation
        index_col(:obj:`array-like`): the array to use as an index. Should match the target data
    ...
    corr_df = correlation_matrix_by_id_column(align_col, id_col, input_col)

    # For efficiency, sort <data> by <id_col> into different dictionary entries immediately
    assets = corr_df.columns
    data = pd.DataFrame(
        np.column_stack((id_col, align_col, input_col, ref_col)),
        columns=["id_col", "align_col", "input_col", "ref_col"],
        index=index_col
    )
    # merge_df = merge_df.set_index(target_df.index if index_col is None else index_col)
    ...
    ret = pd.DataFrame(data.rename(columns={"input_col": "imputed_input_col"}).imputed_input_col)
    ...
    for target_id, target_data in tqdm(iter(asset_dict.items())):  # Target asset refers to data requiring imputation
        ...
        while (num_nan > 0) & (num_neighbors > 0) & (r2_neighbor > r2_threshold):
            ...
            imputed_data = impute_data(
                target_data.input_col,
                target_data.align_col,
                neighbor_data.ref_col,
                neighbor_data.align_col,
                method=method,
                index_col=target_data.index.values
            )

            # Find indices that were imputed (i.e. NaN in <data>, finite in <imputed_data>)
            imputed_bool = ret.loc[imputed_data.index,  "imputed_input_col"].isnull() & np.isfinite(imputed_data)
            ...
            if len(imputed_ind) > 0:  # There is imputed data to update
                ret.loc[imputed_ind, "imputed_input_col"] = imputed_data
