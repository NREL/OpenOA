...
def correlation_matrix_by_id_column(df, align_col, id_col, value_col):
    ...
    corr_df = pd.DataFrame(index=assets,columns=assets, dtype = float, data = np.nan) # Define correlation matrix that is set to NaN by default
    ...
    for t in assets: # Loop through assets
        ...
        for s in assets: # Loop through remaining assets
            ...
            if (merged_df.empty) | (merged_df.shape[0] < 2): # If merged data frame is empty or has only 1 entry, assign NaN correlation
...
def impute_data(target_data, target_value_col, ref_data, ref_value_col, align_col, method = 'linear'):   ### ADD LINEAR FUNCTIONALITY AS DEFAULT, expection otherwise
...
def impute_all_assets_by_correlation(data, input_col, ref_col, align_col, id_col, r2_threshold = 0.7, method = 'linear'):
    ...
    for target_id, target_data in tqdm(asset_dict.iteritems()): # Target asset refers to data requiring imputation
        ...
        while (num_nan > 0) & (num_neighbors > 0) & (r2_neighbor > r2_threshold):  
            ...
            id_neighbor = corr_list.index[len(corr_list) - num_neighbors - 1] # Name of next highest correlated neighbor
