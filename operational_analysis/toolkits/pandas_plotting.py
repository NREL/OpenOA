...
def powerRose_array(project, fig, rect, tid, shift=[0], direction=1):
    ...
    ntick = 3
    ticks = [np.round(np.min(model_eval[tid]) + ((np.max(model_eval[tid]) - np.min(model_eval[tid])) / ntick) * (t + 1))
...
def subplt_c1_c2_flagged(turbine, axarr, c1, c2, flag_cols, flag_value, cmap='Blues', xlim=None, ylim=None, xlabel=None,
                         ylabel=None):
