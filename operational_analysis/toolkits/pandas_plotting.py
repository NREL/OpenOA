...
def powerRose_array(project, fig, rect, tid, shift = [0], direction = 1):
    ...
    Args: 
        ...
        rect(:obj:`list of four scalars`): [left offset, bottom offset, width, height] as fractions of figure width/height
    ...
    for i in range(len(shift)):
        ax_polar.plot((model_eval['winddirection']*direction+shift[i])*np.pi/180, model_eval[tid],linewidth=3.0, label = str(shift[i])+' deg')
    
    ntick = 3
    ticks = [np.round(np.min(model_eval[tid])+((np.max(model_eval[tid])-np.min(model_eval[tid]))/ntick)*(t+1)) for t in np.arange(ntick)]
...
def subplt_c1_c2_flagged(turbine, axarr, c1, c2, flag_cols, flag_value, cmap='Blues', xlim = None, ylim = None, xlabel= None, ylabel = None):
    ...
    axarr.text(xlim[0]+0.1*(xlim[1]-xlim[0]), ylim[0]+0.9*(ylim[1]-ylim[0]), '%.2f%%' %(float(len(flag_indices))/float(len(turbine))*100.))
    return hb
...
def turbine_polar_line(array, theta, r, 
    ...
    ntick = 3
    ticks = [np.round(ax_polar.get_rmin()+((ax_polar.get_rmax()-ax_polar.get_rmin())/ntick)*(t+1)) for t in np.arange(ntick)]
