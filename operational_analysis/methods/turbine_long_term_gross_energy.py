...
class TurbineLongTermGrossEnergy(object):
    ...
    def setup_daily_dict(self):
        return None
        # Get all 3 reanalysis prodcuts loaded as daily
        #merra_daily = project.reanalysis._product['merra2'].df.resample('D').mean()
        #merra_daily.columns = 'merra2_' + merra_daily.columns
        #merra_daily['merra2_winddirection_deg'] = met_data_processing.compute_wind_direction(u = merra_daily['merra2_u_50m'], 
                                                                          #           v = merra_daily['merra2_v_50m'])
        #merra_daily.head()          
