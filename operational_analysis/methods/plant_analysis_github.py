...
class MonteCarloAEP(object):
    ...
    The preprocessing should run in this order:
        ...
        3. Process reanalysis data - add monthly/daily density-corrected wind speeds from several reanalysis products to the
           monthly data frame
    ...
    def __init__(self, plant, uncertainty_meter=0.005, uncertainty_losses=0.05,
                 ...
                 uncertainty_nan_energy=0.01, time_resolution = 'M', ml_model = 'gbm'):
        """
        ...
        self.long_term_sampling_wd = {} 
        self.long_term_sampling_wd_sin = {} 
        self.long_term_sampling_wd_cos = {} 
        self.long_term_sampling_temp = {} 
        ...
        if ml_model not in ['gbm','etr','gam']:
            raise ValueError("ml_model has to either be gbm (Gradient boosting model, default), etr (Extra trees regressor) or gam (Generalized additive model)")
    ...
    def set_regression_data(self, n):
        ...
        return [mc_wind_speed, mc_temperature, mc_wind_direction_sin, mc_wind_direction_cos, mc_gross_norm]  # Return randomly sampled wind speed, wind direction, temperature and normalized gross energy
    ...
    def run_regression(self, n):
        ...
        """
        reg_data = np.array(self.set_regression_data(n))  # Get regression data
        
        ml = MachineLearningSetup(self.ml_model)
        
        # Perform randomized grid search only once for efficiency
        ml.hyper_optimize(np.transpose(reg_data[0:4,:]), reg_data[4,:], n_iter_search = 5, report = False)
        #ml.hyper_optimize(reg_data[0,:].reshape(-1, 1), reg_data[4,:], n_iter_search = 5, report = False)

        # p are fit coefficients i.e. slope and intercept
        # V is covariance matrix of the polynomial coefficient estimates
        ...
        #self._mc_slope[n] = np.float(ml.hyper_optimize.opt_hyp)
    ...
    def run_AEP_monte_carlo(self):
        ...
        if self.time_resolution == 'M':
            ...
            for n in tqdm(np.arange(num_sim)):
                [ws_lt, temp_lt, wd_sin_lt, wd_cos_lt] = self.sample_long_term_reanalysis(self._mc_num_years_windiness[n],
                                                         self._mc_reanalysis_product[n])  # Get long-term wind speeds
                
                ml_model_cv = self.run_regression(n)  # Get slope, intercept from regression
                # Get long-term normalized gross energy by applying regression result to long-term monthly wind speeds
                gross_norm_lt = ml_model_cv.predict(np.transpose([ws_lt, temp_lt, wd_sin_lt, wd_cos_lt]))
            ...
            for n in tqdm(np.arange(num_sim)):
                ...
                ml_model_cv = self.run_regression(n)  # Get slope, intercept from regression
                # Get long-term normalized gross energy by applying regression result to long-term monthly wind speeds
                gross_lt = ml_model_cv.predict(np.transpose([ws_lt, temp_lt, wd_sin_lt, wd_cos_lt]))
                
                # Get long-term normalized gross energy by applying regression result to long-term monthly wind speeds                                                                    
                #gross_por = self._reanalysis_por_avg[self._mc_reanalysis_product[n]].multiply(slope) + intercept
    ...
    def sample_long_term_reanalysis(self, n, r):
        ...
        if (r, n) in self.long_term_sampling:
            ws_monthly_daily = self.long_term_sampling[(r, n)]
            wd_sin_monthly_daily = self.long_term_sampling_wd_sin[(r, n)]
            wd_cos_monthly_daily = self.long_term_sampling_wd_cos[(r, n)]
            temp_monthly_daily = self.long_term_sampling_temp[(r, n)]
            return [ws_monthly_daily, temp_monthly_daily, wd_sin_monthly_daily, wd_cos_monthly_daily]

        # If data hasn't yet been computed, peform the calculation
        ws_df = self._reanalysis_monthly_daily[r].to_frame().dropna()  # Drop NA values from monthly reanalysis data series
        #wd_df = self._reanalysis_monthly_daily[r + '_wd'].to_frame().dropna()
        ...
        if self.time_resolution == 'M':
            ...
            #wd_monthly_daily = wd_data.groupby(wd_data.index.month)[r + '_wd'].mean()
            ...
            #wd_data = wd_df.tail(n * 366)
            temp_data = temp_df.tail(n * 366)
            u_data = u_df.tail(n * 366)
            v_data = v_df.tail(n * 366)
            ws_monthly_daily = ws_data.groupby([(ws_data.index.month),(ws_data.index.day)])[r].mean()  # Get long-term annualized monthly wind speeds
            #wd_monthly_daily = wd_data.groupby([(wd_data.index.month),(wd_data.index.day)])[r + '_wd'].mean()
        ...
        self.long_term_sampling[(r, n)] = ws_monthly_daily
        self.long_term_sampling_wd[(r, n)] = wd_monthly_daily
        self.long_term_sampling_wd_sin[(r, n)] = wd_sin_monthly_daily
        self.long_term_sampling_wd_cos[(r, n)] = wd_cos_monthly_daily
        self.long_term_sampling_temp[(r, n)] = temp_monthly_daily
