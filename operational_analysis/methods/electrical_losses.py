...
class ElectricalLosses(object):
    ...
    def __init__(self, plant, UQ=False, num_sim=20000):
        ...
        if UQ is True:
            ...
            raise ValueError(
                "UQ has to either be True (uncertainty quantification performed, default)\
                or False (uncertainty quantification NOT performed)")
        self.UQ = UQ
    ...
    def run(self, uncertainty_meter=0.005, uncertainty_scada=0.005,
        ...
        assert isinstance(uncertainty_correction_thresh,
                          expected_type), f"uncertainty_correction_thresh must be {expected_type} for UQ={self.UQ}"
        ...
        # Process meter data to daily sums (if time frequency is less than
        # monthly)
        self._monthly_meter = True  # Keep track of reported meter data frequency

        if (self._plant._meter_freq != 'MS') & (
                self._plant._meter_freq != 'M') & (self._plant._meter_freq != '1MS'):
            self.process_meter()
    ...
    def setup_inputs(self):
        ...
        if self.UQ:
            inputs = {
                ...
                "correction_threshold": np.random.randint(self.uncertainty_correction_thresh[0] * 1000,
    ...
    def process_scada(self):
        ...
        scada_sum['count'] = scada_df.groupby(
            scada_df.index)[['energy_kwh']].count()
        self._scada_sum = scada_sum

        # Calculate daily sum of all turbine energy production and count number
        # of entries
        self._scada_daily = scada_sum.resample(
            'D')['energy_kwh'].sum().to_frame()
        ...
        # Store daily SCADA data where all turbines reporting for every time
        # step during the day
        self._scada_sub = self._scada_daily[self._scada_daily['count']
                                            == expected_count]
    ...
    def process_meter(self):
        ...
        self._meter_daily['mcount'] = meter_df.resample('D')[
            'energy_kwh'].count()
        ...
        # Keep only data with all turbines reporting for every time step during
        # the day
        self._meter_daily = self._meter_daily[self._meter_daily['mcount']
                                              == expected_mcount]
    ...
    def calculate_electrical_losses(self):
        ...
        # Loop through number of simulations, calculate losses each time, store
        # results
        for n in tqdm(np.arange(self.num_sim)):
            ...
            # If monthly meter data, sum the corrected daily turbine energy to
            # monthly and merge with meter
            if self._monthly_meter:

                scada_monthly = self._scada_daily.resample(
                    'MS')['corrected_energy'].sum().to_frame()
                ...
                scada_monthly['expected_count_monthly'] = scada_monthly.index.daysinmonth * self._hours_per_day * self._min_per_hour / \
                    (pd.to_timedelta(self._plant._scada_freq).total_seconds() /
                     60) * self._plant._num_turbines
                scada_monthly['perc'] = scada_monthly['count'] / \
                    scada_monthly['expected_count_monthly']

                # Filter out months in which there was less than x% of total
                # running (all turbines at all timesteps)
                scada_monthly = scada_monthly.loc[scada_monthly['perc']
                                                  >= self._run.correction_threshold, :]
                merge_df = meter_df.join(scada_monthly)

            # If sub-monthly meter data, merge the daily data for which all
            # turbines are reporting at all timestamps
            else:
                # Note 'self._scada_sub' only contains full reported data
                merge_df = self._meter_daily.join(self._scada_sub)

            # Drop non-concurrent timestamps and get total sums over concurrent
            # period of record
            merge_df.dropna(inplace=True)
            self._merge_df = merge_df
            merge_sum = merge_df.sum(axis=0)

            # Calculate electrical loss from difference of sum of turbine and
            # meter energy
            self._total_turbine_energy = merge_sum['turbine_energy_kwh'] * \
                self._run.scada_data_fraction
            self._total_meter_energy = merge_sum['energy_kwh'] * \
                self._run.meter_data_fraction

            self._electrical_losses[n] = 1 - \
                self._total_meter_energy / self._total_turbine_energy
