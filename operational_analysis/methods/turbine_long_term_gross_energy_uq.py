...
class TurbineLongTermGrossEnergy(object):
    ...
    @logged_method_call
    def __init__(self, plant, num_sim = 20000, uncertainty_meter=0.005, uncertainty_scada=0.005,
    ...
    @logged_method_call
    def run(self, reanal_subset = ['erai', 'ncep2', 'merra2'], enable_plotting = False,
            plot_dir = None):
        """
    ...
    def filter_turbine_data(self,n):
        """
        ...
        for t in self._turbs:
            ...
            dic[t].loc[:,'flag_bin'] = filters.bin_filter(bin_col = dic[t].loc[:, 'wtur_W_avg'], 
                                                          ...
                                                          threshold = self._mc_wind_bin_thresh[n], # wind bin thresh; 2.5 or so 
                                                          center_type = 'median', 
    ...
    def setup_model_dict(self, n):
        ...
        mod['energy_imputed'] = mod['energy_imputed'] * self._mc_scada_data_fraction[n]
    ...
    def apply_model_to_lt(self,n):
        ...
        print(self._plant_gross[n])


    @logged_method_call
    def setup_TIE_monte_carlo_inputs(self):
        """
        Perform Monte Carlo sampling for reported data
        ...
        self._plant_gross = np.empty(num_sim)
    ...
    def run_TIE_monte_carlo(self):
        """
        Loop through OA process a number of times and return array of TIE results each time
        ...
        for n in tqdm(np.arange(num_sim)):
            ...
            self.filter_turbine_data(n) # Filter turbine data
            ...
            self.setup_daily_reanalysis_data(n) # Setup daily reanalysis products
            
            # MC-sampled parameter in this function!
            logger.info("Processing scada data to daily sums")
            self.filter_sum_impute_scada(n) # Setup daily scada data
            
            # MC-sampled parameter in this function!
            logger.info("Setting up daily data for model fitting")
            self.setup_model_dict(n) # Setup daily data to be fit using the GAM
