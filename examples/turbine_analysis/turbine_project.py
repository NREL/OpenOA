...
class TurbineExampleProject(PlantData):
    ...
    def fix_data_issues(self):
        ...
        def generate_reanal(index):
            ...
            return d
        p._reanalysis._product['merra2'].df = generate_reanal(p.scada.df.index)
        p._reanalysis._product['erai'].df = generate_reanal(p.scada.df.index)
        p._reanalysis._product['ncep2'].df = generate_reanal(p.scada.df.index)
