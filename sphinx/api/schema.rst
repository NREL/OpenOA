...
classes used to contain all data relevant to a wind plant and is used throughout OpenOA. The
:py:class:`openoa.plant.PlantData` holds multiple Pandas Data Frames, each with a specified schema.
...
`OpeOA/openoa/schema/ <https://github.com/NREL/OpenOA/tree/develop_v3/openoa/schema>` in any of the JSON or YAML files.
...
:py:class:`openoa.schema.PlantMetaData` structure, a user can map the column names already present
