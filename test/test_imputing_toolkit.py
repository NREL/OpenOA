...
class SimpleFilters(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_df = pd.DataFrame(data = {'time': ['01', '02', '03', '04', '05','01', '02', '03', '04', '05'], 
                                            'data': [0, np.nan, 4, 5, 8, 13, 18, 20, 20, 30],
                                            'id': ['a', 'a', 'a', 'a', 'a', 'b', 'b', 'b', 'b', 'b']}, 
                                            index = np.arange(10))
        
        # Test dataframe #2: two assets, first asset as 2 NaN entries, 2nd asset as one NaN entry overlapping with first asset
        self.test2_df = pd.DataFrame(data = {'time': ['01', '02', '03', '04', '05','01', '02', '03', '04', '05'], 
                                             'data': [0, np.nan, np.nan, 5, 8, 13, np.nan, 20, 20, 30],
                                             'id': ['a', 'a', 'a', 'a', 'a', 'b', 'b', 'b', 'b', 'b']}, 
                                             index = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j'])
        
        # Test dataframe #3: single asset, no NaN data
        self.test3_df = pd.DataFrame(index = ['a', 'b', 'c', 'd', 'e'], 
                                data = {'data1':[1, 2, 3, 4, 5], 
                                        'align': ['a', 'b', 'c', 'd', 'e']})
        
        # Test dataframe #4: single asset, one less entry than dataframe #3
        self.test4_df = pd.DataFrame(index = np.arange(4), 
                                     data = {'data2':[1, 2, 4, 5], 
                                             'align': ['a', 'c', 'd', 'e']})

        # Test dataframe #5, single asset, 3 NaN data
        self.test5_df = pd.DataFrame(index = ['a', 'b', 'c', 'd', 'e'], 
                                     data = {'data1':[1, np.nan, np.nan, np.nan, 5], 
                                             'align': ['a', 'b', 'c', 'd', 'e']})

        # Test dataframe #6, same as #5 but with values reversed 
        self.test6_df = pd.DataFrame(index = ['a', 'b', 'c', 'd', 'e'], 
                                     data = {'data1':[5, np.nan, np.nan, np.nan, 1], 
                                             'align': ['a', 'b', 'c', 'd', 'e']})
    
        # Test dataframe #7, same as #5 but all values NaN
        self.test7_df = pd.DataFrame(index = ['a', 'b', 'c', 'd', 'e'], 
                                     data = {'data1':[np.nan, np.nan, np.nan, np.nan, np.nan], 
                                             'align': ['a', 'b', 'c', 'd', 'e']})
        
        # Data frame of two assets with no overlapping occurrences of valid data
        self.test9_df = pd.DataFrame(data = {'time': ['01', '02', '03', '04', '05','01', '02', '03', '04', '05'], 
        ...
        self.test10_df = pd.DataFrame(data = {'time': ['01', '02', '03', '04', '05',
                                                      '01', '02', '03', '04', '05',
                                                      '01', '02', '03', '04', '05'], 
                                             'data': [0, np.nan, np.nan, 5, 8, 
                                                      np.nan, 23, 33, np.nan, np.nan,
                                                      20.5, 41, np.nan, 85, np.nan],
                                             'id': ['a', 'a', 'a', 'a', 'a', 
                                                    'b', 'b', 'b', 'b', 'b',
                                                    'c', 'c', 'c', 'c', 'c']}, 
                                             index = np.arange(15))
        ...
        self.test11_df = pd.DataFrame(data = {'time': ['01', '02', '03', '04', '05', '06', '07',
                                                       '01', '02', '03', '04', '05', '06', '07',
                                                       '01', '02', '03', '04', '05', '06', '07'],
                                             'data': [0, np.nan, np.nan, 5, 8, 11, 14,
                                                      np.nan, 23, 33, np.nan, 48, 60, 68,
                                                      20.5, 41, np.nan, 85, np.nan, 120, 145],
                                             'id': ['a', 'a', 'a', 'a', 'a', 'a', 'a',
                                                    'b', 'b', 'b', 'b', 'b', 'b', 'b',
                                                    'c', 'c', 'c', 'c', 'c', 'c', 'c']}, 
                                             index = np.arange(21))
        ...
        self.test12_df = pd.DataFrame(data = {'time': ['01', '02', '03', '04', '05', '06', '07',
                                                       '01', '02', '03', '04', '05', '06', '07'],
                                             'data': [0, np.nan, np.nan, 5, 8, 11, 14,
                                                      40, 40, np.nan, 20, np.nan, 80, 10],
                                             'id': ['a', 'a', 'a', 'a', 'a', 'a', 'a',
                                                    'c', 'c', 'c', 'c', 'c', 'c', 'c']}, 
                                             index = np.arange(14))
    ...
    def test_impute_data(self):
        ...
        y = imputing.impute_data(self.test_df.loc[self.test_df.id=='a'], 'data', self.test_df.loc[self.test_df.id=='b'], 'data', 'time')
        ...
        y2 = imputing.impute_data(self.test2_df.loc[self.test2_df.id=='a'], 'data', self.test2_df.loc[self.test2_df.id=='b'], 'data', 'time')
    ...
    def test_impute_all_assets_by_correlation(self):        
        ...
        nptest.assert_array_almost_equal((y.loc[self.test11_df['data'].isnull(), 'imputed_data']).values, ans.values, decimal = 4)
